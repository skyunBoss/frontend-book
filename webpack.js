...
// npm 嵌套生成node_modules，层级过深会导致文件路径过长的问题，模块实例不能共享
// yarn和pnpm解决node_modules嵌套过深的问题，扁平化处理
// 扁平化算法本身复杂性很高，耗时较长，依赖结构的不确定性
// pnpm复刻了npm所有的命令，所以使用方法和npm一样，并且在安装目录结构上做了优化，特点是善用链接，且由于链接的优势，大多数情况下pnpm的安装速度比yarn和npm更快
// pnpm会把自身和依赖放在.pnpm文件夹下，好处是允许包引用自身，自身和依赖平摊可以避免循环结构

// webpck proxy原理
// proxy工作原理实质上是利用http-proxy-middleware 这个http代理中间件